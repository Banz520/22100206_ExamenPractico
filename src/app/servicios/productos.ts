...
export class Productos {
    ...
    async getProductos(): Promise<Producto[]> {
        try {
            ...
            return [
                ...
                { id: 2, nombre: 'RAM Generica', precio: 800, marca: 'Teléfono inteligente última generación' },
    ...
    private parseXML(xmlString: string): Producto[] {
        try {
            ...
            for (let i = 0; i < nodes.length; i++) { //id, nombre, marca, categoria, precio, imagen(direccion)
                const node = nodes[i];
